...
class rotateList{
    ...
    private static int adjust_rotations_needed(int n, int len) {
        // If n is positive then Math of rotations performed is from right side
        // and if n is negative then Math of rotations performed from left side
        // Let's optimize the Math of rotations.
        // Handle case if 'n' is a negative Math.
        n = n % len;
    ...
    public static LinkedListNode rotate_list(LinkedListNode head, int n) {
        ...
        // If n (Math of rotations required) is bigger than
        // length of linked list or if n is negative then
        // we need to adjust total Math of rotations needed
